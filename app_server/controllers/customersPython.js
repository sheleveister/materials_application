...
connection.connect(function(err) {
    if(!err) {
        console.log("Database Python is connected");    
    } else {
        console.log("Error connecting database Python");    
    }
});
...
exports.list = function(req, res) {
    connection.query('SELECT data FROM materials_courses WHERE id = 2', function(err, rows) {
        ...
        if(err) {
            ...
            res.render('index', {
                title: "Мaтериалы для курса Python",
                image: "/img/python.jpg",
                action: "/python",
                href: "../python/",
                global_path: "js/global_python.js",
                data: data,
                materials: true
            });
        }
...
exports.save = function(req, res, next) {
    ...
    connection.query('SELECT data FROM materials_courses WHERE id = 2', function(err, rows) {
        ...
        connection.query( "UPDATE materials_courses SET `data` = ? WHERE id = 2", [updated_data], function(err, rows) {
            if (err) console.log("Error inserting : %s ", err );
        });

        var path = require('path');
        var dir = path.resolve(".")+'/uploads/python/';
...
exports.delete_customer = function(req, res) {   
    ...
    connection.query('SELECT data FROM materials_courses WHERE id = 2', function(err, rows) {
        ...
        connection.query( "UPDATE materials_courses SET `data` = ? WHERE id = 2", [deleted_data_java],
            function(err, rows) {
                if (err) console.log("Error inserting : %s ", err );
            });
...
exports.file = function(req, res, next) { 
    ...
    var path = path.resolve(".")+'/uploads/python/' + file; // путь откуда берет файл для скачивания
    res.download(path); 
